...
class TestOutcome:
    ...
    # Why, in words. Inside, the same string crosses the log diode to the wired
    # terminal and never returns to the worker (bundle-format-spec §8.5). Outside
    # there is nothing to withhold it from, so recon-check prints it.
    detail: str = ""
...
class CheckReport:
    ...
    def render(self) -> str:
        ...
        for t in self.tests:
            lines.append(f"{t.result:5s}  {t.test_id}  {t.kind:22s}  {t.detail}")
...
def check_package(package_dir: Path) -> CheckReport:
    ...
    with tempfile.TemporaryDirectory(prefix="recon-check-") as scratch:
        ...
        for result in run_suite(site, suite, spec, fixtures, writer="recon-check"):
            report.tests.append(
                TestOutcome(
                    result.test_id,
                    kinds[result.test_id],
                    RESULT_NAMES[result.code],
                    result.detail,
                )
            )
    return report
